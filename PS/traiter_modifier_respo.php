...
if(isset($_POST['envoyer_demande_respo'])){
    ...
    $sql_new_quantity_requested = "UPDATE requested_by_respo SET quantity_requested = $quantity_requested WHERE id_request_respo=".$id_request_respo."";
    if($link_new_quantity_requested = mysqli_query($link, $sql_new_quantity_requested)){
        // The rest of the updates
        $sql_update_status = "UPDATE requested_by_respo SET processing_status = 2 WHERE id_request_respo=".$id_request_respo."";
             ...
             ORDER BY id_product DESC";
                ...
                while($data_product = mysqli_fetch_assoc($link_products)){
                    ...
                    if ($deduct_quantity >= $other_product_quantity) {
                        ...
                        echo "We have $deduct_quantity >= $other_product_quantity for id  $other_product_id";
                        $deduct_quantity = $deduct_quantity - $other_product_quantity;
                        $other_product_quantity = 0;
                        echo "Result is DQ = $deduct_quantity; OPQ = $other_product_quantity for id  $other_product_id";
                    } else {
                    ...
                    if($link_update_quantity = mysqli_query($link, $update_quantity)){
                        echo "Deduction done for $other_product_id of the new quantity $other_product_quantity";
                    }
                ...
                $experation_date = $data_quantity_requested['experation_date'];
                 // Get the destination based on RAsking:
                 ...
                 $id_des = "Stock ".$get_src['stock_name'];
